...
// ─── Horários page (separate component to use hooks safely) ──────────────────
function HorariosPage({ horarios, setHorarios, isEducador, isMobile, S, setShowModal, setNewItem }) {
  const dias = Object.keys(horarios);
  const [selDia, setSelDia] = useState(dias[0]);
  const aulas = horarios[selDia] || [];
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>🕐 Horário Escolar</h2>
        {isEducador && <button style={S.btn("#f59e0b")} onClick={()=>{ setShowModal("editarHorario"); setNewItem({ diaSel:selDia, aula:0, materia:"" }); }}>✏️ Editar</button>}
      </div>
      <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:8, marginBottom:16 }}>
        {dias.map(d=>(
          <button key={d} onClick={()=>setSelDia(d)} style={{ padding:"8px 14px", borderRadius:10, border:"none", cursor:"pointer", fontSize:13, fontWeight:600, flexShrink:0, background:selDia===d?"linear-gradient(135deg,#8b5cf6,#7c3aed)":"rgba(255,255,255,0.07)", color:selDia===d?"white":"#94a3b8" }}>{d}</button>
        ))}
      </div>
      <div style={S.card}>
        <h3 style={{ margin:"0 0 14px", color:"#f1f5f9", fontSize:15, fontWeight:700 }}>📚 {selDia}</h3>
        {aulas.map((mat,i)=>(
          <div key={i} style={{ display:"flex", alignItems:"center", gap:14, padding:"11px 0", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ background:"rgba(139,92,246,0.15)", border:"1px solid rgba(139,92,246,0.3)", borderRadius:10, padding:"6px 10px", textAlign:"center", minWidth:isMobile?48:62, flexShrink:0 }}>
              <div style={{ fontSize:10, color:"#a78bfa", fontWeight:700 }}>{i+1}ª aula</div>
              <div style={{ fontSize:isMobile?8:10, color:"#7c3aed", fontWeight:600 }}>{HORARIOS_AULA[i]}</div>
            </div>
...
// ─── Merenda page ─────────────────────────────────────────────────────────────
function MerendaPage({ merenda, isEducador, isMobile, S, setShowModal, setNewItem }) {
  const [selDia, setSelDia] = useState(merenda[0]?.dia || "Segunda");
  const cardapio = merenda.find(m=>m.dia===selDia);
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>🍽️ Cardápio da Merenda</h2>
        {isEducador && <button style={S.btn("#10b981")} onClick={()=>{ setShowModal("editarMerenda"); setNewItem({ diaMerenda:selDia, novoCardapio:(cardapio?.cardapio||[]).join("\n") }); }}>✏️ Editar</button>}
      </div>
      <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:8, marginBottom:16 }}>
        {merenda.map(m=>(
          <button key={m.dia} onClick={()=>setSelDia(m.dia)} style={{ padding:"8px 14px", borderRadius:10, border:"none", cursor:"pointer", fontSize:13, fontWeight:600, flexShrink:0, background:selDia===m.dia?"linear-gradient(135deg,#10b981,#059669)":"rgba(255,255,255,0.07)", color:selDia===m.dia?"white":"#94a3b8" }}>{m.dia}</button>
        ))}
      </div>
      <div style={S.card}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
          ...
          <div>
            <h3 style={{ margin:0, color:"#f1f5f9", fontSize:17, fontWeight:700 }}>Cardápio de {selDia}</h3>
...
export default function App() {
  ...
  function handleCadastro() {
    ...
    if (chk.tipo !== "aluno") { setCadError(`O cadastro público é apenas para alunos. E-mails de educadores (${EDUCADOR_DOMAIN}) devem ser cadastrados pela direção.`); return; }
  ...
  function handleRecuperar() {
    ...
    if (recStep === 1) {
      ...
      if (recNovaSenha.length < 6) { setRecError("A senha deve ter ao menos 6 caracteres."); return; }
      if (recNovaSenha !== recConfirm) { setRecError("As senhas não coincidem."); return; }
      setUsers(p => p.map(u => u.id===recFoundUser.id ? {...u, senha:recNovaSenha} : u));
      showToast("Senha redefinida!");
      setScreen("login"); setRecStep(1); setRecEmail(""); setRecNovaSenha(""); setRecConfirm(""); setRecFoundUser(null);
    }
  }
  ...
  function handleAddItem() {
    if (showModal==="aviso") {
      if (!newItem.titulo||!newItem.descricao) { showToast("Preencha todos os campos.","error"); return; }
      setAvisos(p=>[...p, { id:Date.now(), ...newItem, data:new Date().toISOString().split("T")[0], autor:user.nome }]);
      showToast("Aviso publicado!");
    } else if (showModal==="comunicado") {
      if (!newItem.titulo||!newItem.descricao) { showToast("Preencha todos os campos.","error"); return; }
      setComunicados(p=>[...p, { id:Date.now(), ...newItem, data:new Date().toISOString().split("T")[0], autor:user.nome }]);
      showToast("Comunicado publicado!");
    } else if (showModal==="evento") {
      if (!newItem.nome||!newItem.data||!newItem.tipo) { showToast("Preencha todos os campos.","error"); return; }
      setEventos(p=>[...p, { id:Date.now(), ...newItem, turma:newItem.turma||"Todas", cor:tc(newItem.tipo) }]);
      showToast("Evento adicionado!");
    } else if (showModal==="novoUsuario") {
      const email = (newItem.email||"").toLowerCase().trim();
      if (!newItem.nome||!email||!newItem.senha||!newItem.tipo) { showToast("Preencha todos os campos.","error"); return; }
      const chk = validateEmail(email);
      if (!chk.valid) { showToast(`E-mail deve terminar em ${ALUNO_DOMAIN} ou ${EDUCADOR_DOMAIN}`,"error"); return; }
      if (users.find(u=>u.email===email)) { showToast("E-mail já cadastrado.","error"); return; }
      const extra = newItem.tipo==="aluno" ? { turma:newItem.extra||"Sem turma" }
                  : newItem.tipo==="professor" ? { disciplina:newItem.extra||"" }
                  : { cargo:newItem.extra||"" };
      setUsers(p=>[...p, { id:Date.now(), nome:newItem.nome.trim(), email, senha:newItem.senha, tipo:newItem.tipo, ...extra }]);
      showToast("Usuário cadastrado!");
    } else if (showModal==="editarHorario") {
      const {diaSel,aula,materia}=newItem;
      if (!diaSel||aula===undefined||!materia) { showToast("Preencha todos os campos.","error"); return; }
      setHorarios(prev=>{
        const n={...prev};
        const arr=[...(n[diaSel]||[])];
        arr[aula]=materia;
      ...
      const {diaMerenda,novoCardapio}=newItem;
      if (!diaMerenda||!novoCardapio) { showToast("Preencha os campos.","error"); return; }
      const itens=novoCardapio.split("\n").map(s=>s.trim()).filter(Boolean);
      setMerenda(p=>p.map(m=>m.dia===diaMerenda?{...m,cardapio:itens}:m));
      showToast("Cardápio atualizado!");
    }
    setShowModal(null); setNewItem({});
  }

  // ── Styles ──────────────────────────────────────────────────────────────────
  const S = {
    input: { width:"100%", padding:"12px 16px", background:"rgba(255,255,255,0.07)", border:"1px solid rgba(139,92,246,0.3)", borderRadius:12, color:"#e2e8f0", fontSize:15, outline:"none", boxSizing:"border-box" },
    label: { fontSize:13, fontWeight:600, color:"#94a3b8", display:"block", marginBottom:6 },
    error: { background:"rgba(239,68,68,0.15)", border:"1px solid rgba(239,68,68,0.3)", borderRadius:10, padding:"10px 14px", color:"#f87171", fontSize:14, marginBottom:14 },
    btnPrimary: { width:"100%", padding:14, background:"linear-gradient(135deg,#8b5cf6,#7c3aed)", border:"none", borderRadius:12, color:"white", fontSize:15, fontWeight:700, cursor:"pointer" },
    btnSecondary: { width:"100%", padding:13, background:"transparent", border:"1px solid rgba(139,92,246,0.4)", borderRadius:12, color:"#a78bfa", fontSize:14, fontWeight:600, cursor:"pointer", marginTop:10 },
    badge: (tipo) => ({ display:"inline-flex", alignItems:"center", padding:"3px 10px", background:tc(tipo)+"22", color:tc(tipo), borderRadius:20, fontSize:11, fontWeight:700, border:`1px solid ${tc(tipo)}44` }),
    btn: (color="#8b5cf6") => ({ padding:"9px 18px", background:color+"22", border:`1px solid ${color}44`, borderRadius:10, color, fontSize:14, fontWeight:600, cursor:"pointer", display:"inline-flex", alignItems:"center", gap:6 }),
    card: { background:"rgba(30,27,75,0.45)", border:"1px solid rgba(139,92,246,0.15)", borderRadius:16, padding:isMobile?16:22, marginBottom:16 },
    overlay: { position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", backdropFilter:"blur(4px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:16 },
    modal: { background:"#1a1740", border:"1px solid rgba(139,92,246,0.4)", borderRadius:20, padding:isMobile?20:32, width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto", boxSizing:"border-box" },
  };
  ...
  const AuthBg = ({ children }) => (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%)", position:"relative", overflow:"hidden", padding:16 }}>
      ...
      <div style={{ background:"rgba(30,27,75,0.65)", backdropFilter:"blur(20px)", border:"1px solid rgba(139,92,246,0.3)", borderRadius:24, padding:isMobile?"32px 22px":"44px 40px", width:"100%", maxWidth:440, position:"relative", zIndex:1, boxSizing:"border-box" }}>
        {children}
      </div>
      {toast && <div style={{ position:"fixed", bottom:24, right:16, background:toast.type==="error"?"rgba(239,68,68,0.93)":"rgba(16,185,129,0.93)", backdropFilter:"blur(10px)", borderRadius:12, padding:"12px 18px", color:"white", fontWeight:600, fontSize:14, zIndex:300, maxWidth:280 }}>{toast.type==="error"?"⚠️":"✅"} {toast.msg}</div>}
    </div>
  );
  ...
  const AppLogo = () => (
    <div style={{ textAlign:"center", marginBottom:26 }}>
      ...
      <h1 style={{ fontSize:isMobile?22:26, fontWeight:800, color:"#f1f5f9", margin:"0 0 4px", letterSpacing:-0.5 }}>EduConnect</h1>
  ...
  if (screen==="login") return (
    <AuthBg>
      ...
      <input style={{ ...S.input, marginBottom:14 }} placeholder={`usuario${ALUNO_DOMAIN}`} value={loginEmail} onChange={e=>setLoginEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
      <label style={S.label}>Senha</label>
      <div style={{ position:"relative", marginBottom:20 }}>
        <input type={showLoginPwd?"text":"password"} style={{ ...S.input, paddingRight:48 }} placeholder="••••••" value={loginSenha} onChange={e=>setLoginSenha(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()}/>
        <button onClick={()=>setShowLoginPwd(v=>!v)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#64748b", cursor:"pointer", fontSize:16 }}>{showLoginPwd?"🙈":"👁️"}</button>
      </div>
      ...
      <div style={{ marginTop:14, display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
        <span style={{ color:"#64748b", fontSize:13 }}>Não tem conta? <span style={{ color:"#a78bfa", cursor:"pointer", fontWeight:600 }} onClick={()=>{ setScreen("cadastro"); setCadError(""); }}>Cadastre-se</span></span>
        <span style={{ color:"#a78bfa", fontSize:13, cursor:"pointer", fontWeight:600 }} onClick={()=>{ setScreen("recuperar"); setRecStep(1); setRecError(""); setRecEmail(""); }}>Esqueci a senha</span>
      </div>
    </AuthBg>
  );
  ...
  if (screen==="cadastro") return (
    <AuthBg>
      ...
      <div style={{ background:"rgba(139,92,246,0.1)", border:"1px solid rgba(139,92,246,0.25)", borderRadius:10, padding:"10px 14px", marginBottom:18, fontSize:13, color:"#a78bfa", lineHeight:1.5 }}>
        📌 Cadastro público disponível apenas para <strong>alunos</strong> com e-mail <strong>{ALUNO_DOMAIN}</strong>.<br/>
        Professores, coordenadores e direção são cadastrados pela administração.
      </div>
      <label style={S.label}>Nome completo</label>
      <input style={{ ...S.input, marginBottom:12 }} placeholder="Seu nome" value={cadNome} onChange={e=>setCadNome(e.target.value)}/>
      <label style={S.label}>E-mail de aluno</label>
      <input style={{ ...S.input, marginBottom:12 }} placeholder={`usuario${ALUNO_DOMAIN}`} value={cadEmail} onChange={e=>setCadEmail(e.target.value)}/>
      <label style={S.label}>Turma</label>
      <input style={{ ...S.input, marginBottom:12 }} placeholder="Ex: 3º A" value={cadTurma} onChange={e=>setCadTurma(e.target.value)}/>
      <label style={S.label}>Senha (mín. 6 caracteres)</label>
      <div style={{ position:"relative", marginBottom:12 }}>
        <input type={showCadPwd?"text":"password"} style={{ ...S.input, paddingRight:48 }} placeholder="••••••" value={cadSenha} onChange={e=>setCadSenha(e.target.value)}/>
        <button onClick={()=>setShowCadPwd(v=>!v)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#64748b", cursor:"pointer", fontSize:16 }}>{showCadPwd?"🙈":"👁️"}</button>
      </div>
      <label style={S.label}>Confirmar senha</label>
      <input type="password" style={{ ...S.input, marginBottom:18 }} placeholder="••••••" value={cadSenha2} onChange={e=>setCadSenha2(e.target.value)}/>
      {cadError && <div style={S.error}>{cadError}</div>}
      <button style={S.btnPrimary} onClick={handleCadastro}>Criar Conta</button>
      <button style={S.btnSecondary} onClick={()=>setScreen("login")}>← Voltar para Login</button>
    </AuthBg>
  );
    ...
    <AuthBg>
      ...
      {recStep===1 ? (
        <>
          ...
          <label style={S.label}>E-mail institucional</label>
          <input style={{ ...S.input, marginBottom:18 }} placeholder={`usuario${ALUNO_DOMAIN}`} value={recEmail} onChange={e=>setRecEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleRecuperar()}/>
          {recError && <div style={S.error}>{recError}</div>}
          <button style={S.btnPrimary} onClick={handleRecuperar}>Verificar E-mail</button>
        </>
      ) : (
        <>
          <p style={{ color:"#94a3b8", fontSize:13, marginBottom:20, textAlign:"center" }}>Conta encontrada: <strong style={{color:"#a78bfa"}}>{recFoundUser?.nome}</strong>. Defina a nova senha.</p>
          <label style={S.label}>Nova senha (mín. 6 caracteres)</label>
          <input type="password" style={{ ...S.input, marginBottom:12 }} placeholder="Nova senha" value={recNovaSenha} onChange={e=>setRecNovaSenha(e.target.value)}/>
          <label style={S.label}>Confirmar nova senha</label>
          <input type="password" style={{ ...S.input, marginBottom:18 }} placeholder="Repita" value={recConfirm} onChange={e=>setRecConfirm(e.target.value)}/>
          {recError && <div style={S.error}>{recError}</div>}
          <button style={S.btnPrimary} onClick={handleRecuperar}>Redefinir Senha</button>
        </>
      )}
      <button style={S.btnSecondary} onClick={()=>setScreen("login")}>← Voltar para Login</button>
    </AuthBg>
  ...
  function renderContent() {
    ...
    if (activeTab==="inicio") return (
      <div>
        <div style={{ background:"linear-gradient(135deg,rgba(139,92,246,0.2),rgba(124,58,237,0.1))", border:"1px solid rgba(139,92,246,0.2)", borderRadius:20, padding:isMobile?"18px":"26px 30px", marginBottom:18, display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
          <div>
            <p style={{ color:"#a78bfa", fontSize:12, fontWeight:600, margin:0 }}>Bem-vindo(a)</p>
            <h2 style={{ fontSize:isMobile?20:26, fontWeight:800, color:"#f1f5f9", margin:"4px 0 6px", letterSpacing:-0.5 }}>{user.nome}</h2>
            <p style={{ color:"#64748b", fontSize:13, margin:0 }}>{user.tipo==="aluno"?`Turma: ${user.turma}`:user.tipo==="professor"?`Disciplina: ${user.disciplina||""}`:user.cargo||roleLabel(user.tipo)}</p>
          </div>
          <div style={{ width:isMobile?50:62, height:isMobile?50:62, flexShrink:0, borderRadius:16, background:"linear-gradient(135deg,#8b5cf6,#7c3aed)", display:"flex", alignItems:"center", justifyContent:"center", color:"white" }}>
            <RoleIcon tipo={user.tipo} size={isMobile?24:30}/>
          </div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:isMobile?10:14, marginBottom:18 }}>
          {[{label:"Avisos",v:avisos.length,color:"#3b82f6",icon:"📢"},{label:"Comunicados",v:comunicados.length,color:"#ec4899",icon:"📋"},{label:"Eventos",v:eventos.filter(e=>e.data>="2026-06-04").length,color:"#8b5cf6",icon:"📅"}].map(s=>(
            <div key={s.label} style={{ background:`${s.color}11`, border:`1px solid ${s.color}33`, borderRadius:14, padding:isMobile?"12px 8px":"18px 18px", textAlign:"center" }}>
              <div style={{ fontSize:isMobile?20:24, marginBottom:4 }}>{s.icon}</div>
              <div style={{ fontSize:isMobile?22:28, fontWeight:800, color:s.color }}>{s.v}</div>
              <div style={{ fontSize:isMobile?10:12, color:"#64748b", fontWeight:500 }}>{s.label}</div>
            </div>
          ))}
        </div>
        <div style={S.card}>
          ...
          {[...avisos].reverse().slice(0,3).map(a=>(
            <div key={a.id} style={{ padding:"11px 0", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                <span style={S.badge("aviso")}>Aviso</span>
        ...
        <div style={S.card}>
          ...
          {eventos.filter(e=>e.data>="2026-06-04").slice(0,4).map(ev=>(
            <div key={ev.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ background:ev.cor+"22", border:`1px solid ${ev.cor}44`, borderRadius:10, padding:"5px 10px", textAlign:"center", minWidth:isMobile?42:52, flexShrink:0 }}>
              ...
              <div style={{ minWidth:0 }}>
                ...
                <span style={S.badge(ev.tipo)}>{tl(ev.tipo)} · {ev.turma}</span>
              </div>
            </div>
          ))}
        </div>
    ...
    if (activeTab==="avisos") return (
      <div>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
          <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>📢 Avisos</h2>
          {isEducador && <button style={S.btn("#3b82f6")} onClick={()=>{ setShowModal("aviso"); setNewItem({}); }}>+ Novo Aviso</button>}
        </div>
        {avisos.length===0 && <p style={{color:"#64748b"}}>Nenhum aviso no momento.</p>}
        {[...avisos].reverse().map(a=>(
          <div key={a.id} style={{ ...S.card, borderLeft:"4px solid #3b82f6" }}>
            <span style={S.badge("aviso")}>Aviso</span>
    ...
    if (activeTab==="comunicados") return (
      <div>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
          <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>📋 Comunicados da Direção</h2>
          {(user.tipo==="direcao"||user.tipo==="coordenador") && <button style={S.btn("#ec4899")} onClick={()=>{ setShowModal("comunicado"); setNewItem({}); }}>+ Novo</button>}
        </div>
        {comunicados.length===0 && <p style={{color:"#64748b"}}>Nenhum comunicado.</p>}
        {[...comunicados].reverse().map(c=>(
          <div key={c.id} style={{ ...S.card, borderLeft:"4px solid #ec4899" }}>
            <span style={S.badge("comunicado")}>Comunicado Oficial</span>
    ...
    if (activeTab==="calendario") {
      ...
      return (
        <div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
            <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>📅 Calendário</h2>
            {isEducador && <button style={S.btn("#8b5cf6")} onClick={()=>{ setShowModal("evento"); setNewItem({}); }}>+ Evento</button>}
          </div>
          <div style={S.card}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
              <button style={{ ...S.btn(), padding:"6px 12px", fontSize:13 }} onClick={()=>setCurrentDate(d=>new Date(d.getFullYear(),d.getMonth()-1,1))}>‹</button>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:isMobile?2:4, marginBottom:4 }}>
              {DIAS.map(d=><div key={d} style={{ textAlign:"center", fontSize:isMobile?10:12, fontWeight:700, color:"#64748b", padding:"4px 0" }}>{d}</div>)}
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:isMobile?2:4 }}>
              {days.map((day,i)=>{
                ...
                return (
                  <div key={i} style={{ minHeight:isMobile?46:66, borderRadius:8, padding:isMobile?3:6, background:isToday?"rgba(139,92,246,0.2)":"rgba(255,255,255,0.03)", border:isToday?"1px solid rgba(139,92,246,0.5)":"1px solid rgba(255,255,255,0.05)" }}>
                    {day && <>
                      <div style={{ fontSize:isMobile?11:13, fontWeight:isToday?800:500, color:isToday?"#a78bfa":"#94a3b8", marginBottom:2 }}>{day}</div>
                      {de.slice(0,isMobile?1:2).map(ev=>(
                        <div key={ev.id} title={ev.nome} style={{ fontSize:9, background:ev.cor+"33", color:ev.cor, borderRadius:3, padding:"1px 3px", marginBottom:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", fontWeight:600 }}>{isMobile?"●":ev.nome}</div>
                      ))}
                      {de.length>(isMobile?1:2)&&<div style={{fontSize:9,color:"#64748b"}}>+{de.length-(isMobile?1:2)}</div>}
                    </>}
                  </div>
                );
              })}
          ...
          {eventos.filter(e=>e.data.startsWith(`${currentDate.getFullYear()}-${String(currentDate.getMonth()+1).padStart(2,'0')}`)).map(ev=>(
            <div key={ev.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", marginBottom:8, background:ev.cor+"11", border:`1px solid ${ev.cor}22`, borderRadius:12 }}>
              ...
              <span style={S.badge(ev.tipo)}>{tl(ev.tipo)}</span>
            </div>
          ))}
        </div>
      );
    ...
    // ── HORÁRIOS (componente separado para hooks) ──────────────────────
    if (activeTab==="horarios") return (
      <HorariosPage horarios={horarios} setHorarios={setHorarios} isEducador={isEducador} isMobile={isMobile} S={S} setShowModal={setShowModal} setNewItem={setNewItem}/>
    );

    // ── MERENDA (componente separado para hooks) ───────────────────────
    if (activeTab==="merenda") return (
      <MerendaPage merenda={merenda} isEducador={isEducador} isMobile={isMobile} S={S} setShowModal={setShowModal} setNewItem={setNewItem}/>
    );

    // ── GERENCIAR ─────────────────────────────────────────────────────
    if (activeTab==="gerenciar") return (
      <div>
        <h2 style={{ margin:"0 0 20px", fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(3,1fr)", gap:14 }}>
          {[
            ...
            <div key={it.modal} style={{ ...S.card, cursor:"pointer", textAlign:"center" }} onClick={()=>{ setShowModal(it.modal); setNewItem({ diaSel:"Segunda", aula:0, materia:"", diaMerenda:"Segunda", novoCardapio:"" }); }}>
    ...
    if (activeTab==="usuarios") return (
      <div>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
          <h2 style={{ margin:0, fontSize:isMobile?20:24, fontWeight:800, color:"#f1f5f9" }}>👥 Usuários</h2>
          <button style={S.btn("#8b5cf6")} onClick={()=>{ setShowModal("novoUsuario"); setNewItem({}); }}>+ Cadastrar</button>
        </div>
    ...
    <div style={{ fontFamily:"'DM Sans',system-ui,sans-serif", minHeight:"100vh", background:"#0f172a", color:"#e2e8f0" }}>
      ...
      <nav style={{ background:"rgba(15,23,42,0.97)", backdropFilter:"blur(20px)", borderBottom:"1px solid rgba(139,92,246,0.2)", padding:"0 14px", display:"flex", alignItems:"center", justifyContent:"space-between", height:58, position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontWeight:800, fontSize:isMobile?15:17, color:"#f1f5f9", letterSpacing:-0.5 }}>EduConnect</span>
        </div>
      ...
      {/* MOBILE BACKDROP */}
      {isMobile && sideOpen && <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", zIndex:150 }} onClick={()=>setSideOpen(false)}/>}

      {/* SIDEBAR */}
      <aside style={{ width:SIDEBAR_W, background:"rgba(15,23,42,0.92)", borderRight:"1px solid rgba(139,92,246,0.15)", padding:"18px 12px", display:"flex", flexDirection:"column", gap:2, position:"fixed", top:isMobile?0:58, left:isMobile?(sideOpen?0:-SIDEBAR_W-10):0, bottom:0, zIndex:isMobile?160:50, transition:"left 0.25s ease", overflowY:"auto" }}>
        {isMobile && (
      ...
      {/* MAIN */}
      <main style={{ marginLeft:isMobile?0:SIDEBAR_W, padding:isMobile?"18px 14px 88px":"26px 28px 48px", minHeight:"calc(100vh - 58px)" }}>
        {renderContent()}
      </main>
        ...
        <div style={S.overlay} onClick={()=>setShowModal(null)}>
          <div style={S.modal} onClick={e=>e.stopPropagation()}>
            ...
            {(showModal==="aviso"||showModal==="comunicado") && <>
              ...
              <input style={{ ...S.input, marginBottom:12 }} placeholder="Título" value={newItem.titulo||""} onChange={e=>setNewItem(n=>({...n,titulo:e.target.value}))}/>
              <label style={S.label}>Descrição</label>
              <textarea style={{ ...S.input, minHeight:90, resize:"vertical", marginBottom:20 }} placeholder="Descreva..." value={newItem.descricao||""} onChange={e=>setNewItem(n=>({...n,descricao:e.target.value}))}/>
            </>}

            {showModal==="evento" && <>
              <label style={S.label}>Nome do Evento</label>
              <input style={{ ...S.input, marginBottom:12 }} placeholder="Nome" value={newItem.nome||""} onChange={e=>setNewItem(n=>({...n,nome:e.target.value}))}/>
              <label style={S.label}>Data</label>
              <input type="date" style={{ ...S.input, marginBottom:12 }} value={newItem.data||""} onChange={e=>setNewItem(n=>({...n,data:e.target.value}))}/>
              <label style={S.label}>Tipo</label>
              <select style={{ ...S.input, marginBottom:12 }} value={newItem.tipo||""} onChange={e=>setNewItem(n=>({...n,tipo:e.target.value}))}>
              ...
              <label style={S.label}>Turma</label>
              <input style={{ ...S.input, marginBottom:20 }} placeholder="Ex: 3º A, Todas" value={newItem.turma||""} onChange={e=>setNewItem(n=>({...n,turma:e.target.value}))}/>
            </>}
            ...
            {showModal==="novoUsuario" && <>
              ...
              <input style={{ ...S.input, marginBottom:12 }} placeholder="Nome" value={newItem.nome||""} onChange={e=>setNewItem(n=>({...n,nome:e.target.value}))}/>
              <label style={S.label}>E-mail institucional</label>
              <input style={{ ...S.input, marginBottom:12 }} placeholder={`email${ALUNO_DOMAIN}`} value={newItem.email||""} onChange={e=>setNewItem(n=>({...n,email:e.target.value}))}/>
              <label style={S.label}>Senha provisória</label>
              <input type="password" style={{ ...S.input, marginBottom:12 }} placeholder="Mín. 6 caracteres" value={newItem.senha||""} onChange={e=>setNewItem(n=>({...n,senha:e.target.value}))}/>
              <label style={S.label}>Tipo</label>
              <select style={{ ...S.input, marginBottom:12 }} value={newItem.tipo||""} onChange={e=>setNewItem(n=>({...n,tipo:e.target.value}))}>
                <option value="">Selecione...</option>
              ...
              <label style={S.label}>{newItem.tipo==="professor"?"Disciplina":newItem.tipo==="aluno"?"Turma":"Cargo"}</label>
              <input style={{ ...S.input, marginBottom:20 }} placeholder="Ex: 3º A / Matemática / Coordenadora" value={newItem.extra||""} onChange={e=>setNewItem(n=>({...n,extra:e.target.value}))}/>
            </>}

            {showModal==="editarHorario" && <>
              <label style={S.label}>Dia da semana</label>
              <select style={{ ...S.input, marginBottom:12 }} value={newItem.diaSel||"Segunda"} onChange={e=>setNewItem(n=>({...n,diaSel:e.target.value}))}>
                {Object.keys(horarios).map(d=><option key={d} value={d}>{d}</option>)}
              </select>
              <label style={S.label}>Número da aula</label>
              <select style={{ ...S.input, marginBottom:12 }} value={newItem.aula||0} onChange={e=>setNewItem(n=>({...n,aula:parseInt(e.target.value)}))}>
                {HORARIOS_AULA.map((h,i)=><option key={i} value={i}>{i+1}ª aula — {h}</option>)}
              </select>
              <label style={S.label}>Disciplina</label>
              <input style={{ ...S.input, marginBottom:20 }} placeholder="Nome da disciplina" value={newItem.materia||""} onChange={e=>setNewItem(n=>({...n,materia:e.target.value}))}/>
            </>}

            {showModal==="editarMerenda" && <>
              <label style={S.label}>Dia</label>
              <select style={{ ...S.input, marginBottom:12 }} value={newItem.diaMerenda||"Segunda"} onChange={e=>setNewItem(n=>({...n,diaMerenda:e.target.value}))}>
                {merenda.map(m=><option key={m.dia} value={m.dia}>{m.dia}</option>)}
              </select>
              <label style={S.label}>Itens do cardápio (um por linha)</label>
              <textarea style={{ ...S.input, minHeight:120, resize:"vertical", marginBottom:20 }} placeholder={"Arroz\nFeijão\nFrango grelhado"} value={newItem.novoCardapio||""} onChange={e=>setNewItem(n=>({...n,novoCardapio:e.target.value}))}/>
            </>}

            <div style={{ display:"flex", gap:10 }}>
              <button style={{ ...S.btn("#8b5cf6"), flex:1, justifyContent:"center" }} onClick={handleAddItem}>
                {showModal==="novoUsuario"?"Cadastrar":showModal.startsWith("editar")?"Salvar":"Publicar"}
              </button>
              <button style={{ ...S.btn("#ef4444"), justifyContent:"center" }} onClick={()=>{ setShowModal(null); setNewItem({}); }}>✕</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position:"fixed", bottom:isMobile?84:24, right:16, background:toast.type==="error"?"rgba(239,68,68,0.93)":"rgba(16,185,129,0.93)", backdropFilter:"blur(10px)", border:"1px solid rgba(255,255,255,0.2)", borderRadius:12, padding:"11px 18px", color:"white", fontWeight:600, fontSize:14, zIndex:400, maxWidth:280 }}>
